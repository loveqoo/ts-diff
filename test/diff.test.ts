...
describe('diff test', () => {
  const wrappedDiff = (a: unknown, b: unknown) => {
  ...
  test('Diff value - object', () => {
    expect(
      wrappedDiff({a: 1, b: 'hello', c: false}, {a: 1, b: 'hello', c: false})
        .length
    ).toEqual(0);
    expect(
      wrappedDiff({a: 1, b: 'hello', c: false}, {a: 2, b: 'bye', c: true})
        .length
    ).toEqual(3);
    ...
    expect(wrappedDiff({a: 1, b: 'hello', c: false}, undefined).length).toEqual(
      1
    );
